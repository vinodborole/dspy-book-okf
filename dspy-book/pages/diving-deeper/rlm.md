...
The model’s code is untrusted, so it runs in a sandbox. The default `PythonInterpreter` executes Python in a Deno and Pyodide WASM runtime with no filesystem or network access, created fresh per `forward()` and shut down afterward. You can pass any `CodeInterpreter` through `interpreter=`, such as a mock for tests or a remote sandbox like E2B or Modal for heavier work.
...
`dspy.Tool` objects. RLM normalizes each to a `Tool`, rejects names that aren’t valid identifiers or that collide with the built-ins, and documents their signatures in the action prompt. The model calls them as ordinary Python inside its code.** interpreter=...**
Any 

`CodeInterpreter`. RLM injects the query tools and output-field metadata into it on each `forward()`, so a custom interpreter still gets `llm_query` and typed submission. Reach for this to run a remote sandbox or a test mock.** dspy.SandboxSerializable**
...
1. Source page: https://dspy.ai/diving-deeper/rlm
