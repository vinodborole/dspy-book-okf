...
Shopify converted a single-prompt GPT-5 task to DSPy, moved to a small Qwen model, and optimized with GEPA, yielding a solution \~75x cheaper and \~2x more reliable. Dropbox used DSPy and prompt optimizers to move to a smaller model and double program accuracy, labeling “10-100 times more data at the same cost.”
...
We can use the natural-language processing library spaCy to assist with most of these measures.
...
See GEPA in depth for the full mechanics: Pareto sampling, per-predictor feedback, `auto` budget translation, and the `detailed_results` audit trail.
