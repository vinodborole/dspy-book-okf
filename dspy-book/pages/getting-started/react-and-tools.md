...
For example, let’s define a tool that lets an agent search Wikipedia using the `wikipedia` library (run `pip install wikipedia`, if you’re following along):
...
For tool authoring patterns, MCP integration, and trajectory debugging, see Tools, ReAct, and MCP.

**Next:** Composing modules →
